...
async function run() {
  ...
  for (const bad of badEmails) {
    ...
    let err = null;
    try { await authClient.signup(bad, 'password123'); } catch (e) { err = e; }
    assert(err !== null, `signup('${bad}') throws`);
    assert(!netCalled, `signup('${bad}') does not call network`);
    assert((err?.message || '').includes('이메일') || (err?.message || '').includes('올바른') || err?.code === 'invalid_email',
  ...
  let e1 = null; try { await authClient.signup('d@t.com', 'pass1234'); } catch (e) { e1 = e; }
  ...
  let e2 = null; try { await authClient.signup('d2@t.com', 'pass1234'); } catch (e) { e2 = e; }
  ...
  let e3 = null; try { await authClient.signup('good@t.com', 'ab'); } catch (e) { e3 = e; }
  assert(e3?.code === 'password_too_short', 'C3: short password → password_too_short', `got: ${e3?.code}`);
